...
const ProgressBar = () => {
  ...
  useEffect(() => {
    ...
    //create a gsap animation that update the width of the progress bar based on the scroll position
    gsap.to(progressFillRef.current, {
      ...
      scrollTrigger: {
        ...
        scrub: 0.3,
        onUpdate: (self) => {
          //optional: You can add additiional effects based on progress
  ...
  return (
    <div ref={progressBar} className='fixed top-0 left-0 w-full h-[5px] bg-gray-800 z-50'>
      <div ref={progressFillRef} className='h-full w-0 bg-[#A1045a] transition-colors duration-300'
        style={{ width: "0%" }} />
    </div>
  )
}
